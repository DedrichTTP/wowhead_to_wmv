...
async function takeScreenshot(url, imagePath, characterType) {
    ...
    if(characterType == "custom"){
        ...
        await page.evaluate(() => {
         ...
         let elToRemove1 = document.querySelector('body > div.lightbox-outer > div > div > div.lightbox-content.modelviewer-modelbg > div > a.fa.fa-arrows-alt');
         if (elToRemove1) elToRemove1.parentNode.removeChild(elToRemove1);
         let elToRemove2 = document.querySelector('body > div.lightbox-outer > div > div > div.lightbox-content.modelviewer-modelbg > div > a.fa.fa-pause');
         if (elToRemove2) elToRemove2.parentNode.removeChild(elToRemove2);
         
         });

        element = await page.$('body > div.lightbox-outer > div > div > div.lightbox-content.modelviewer-modelbg > div > canvas');
    }
